...
export default function Stands() {
  ...
  const filtered = (stands || []).filter((s) => {
    ...
    const matchSearch = !search || s.code?.toLowerCase().includes(search.toLowerCase()) || s.address?.toLowerCase().includes(search.toLowerCase());
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input placeholder="بحث بالكود أو العنوان..." value={search} onChange={(e) => setSearch(e.target.value)} className="ps-9" />
        </div>
            ...
            return (
              <button key={stand.id} onClick={() => navigate(`/stands/${stand.id}`)} className="text-start bg-card border border-border rounded-2xl overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 card-hover group">
                ...
                <div className={cn("p-4 space-y-2", isInactive && "opacity-60")}>
                  ...
                  {(() => {
                    // Show active contract first, fall back to upcoming, then any contract
                    const activeContract = contracts.find((c) => c.stand_id === stand.id && computeContractStatus(c.start_date, c.end_date, c.status) === "active");
                    const upcomingContract = contracts.find((c) => c.stand_id === stand.id && computeContractStatus(c.start_date, c.end_date, c.status) === "upcoming");
                    const contract = activeContract || upcomingContract || contracts.find((c) => c.stand_id === stand.id);
                    const hasContract = !!contract;
                    const isTerminated = hasContract && computeContractStatus(contract.start_date, contract.end_date, contract.status) === "terminated";
                    const isExpired = hasContract && computeContractStatus(contract.start_date, contract.end_date, contract.status) === "expired";
                    ...
                    let owed = 0;
                    let prepaid = 0;
                    const contractStatus = contract ? computeContractStatus(contract.start_date, contract.end_date, contract.status) : null;
                    ...
                    return (
                      <>
                        <div className="flex items-center gap-1.5">
                          <User className={cn("w-3.5 h-3.5 flex-shrink-0", !hasContract || isTerminated || isExpired ? "text-muted-foreground" : "text-primary")} />
                          <p className={cn("text-xs font-medium", !hasContract || isTerminated || isExpired ? "text-muted-foreground" : "text-primary")}>
                            {contract?.clients?.name || "—"}
                            {(isTerminated || isExpired) && " (أخر عميل)"}
                          </p>
                  ...
                  {stand.gov_rental_end && (
                    <div className="flex items-center gap-1.5 text-muted-foreground">
                      ...
                      <p className="text-xs">
                        ...
                        {(() => {
                          const gd = Math.ceil((new Date(stand.gov_rental_end) - new Date()) / 86400000);
                          return gd > 0 && gd <= 90 ? ` (${gd} يوم)` : "";
                        })()}
                      </p>
                    </div>
                  )}
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-[60vh] overflow-y-auto">
            ...
            <FormField label="تاريخ بداية الترخيص">
              <DateInput value={form.gov_rental_start} onChange={(e) => setForm((prev) => ({ ...prev, gov_rental_start: e.target.value }))} placeholder="اختر التاريخ" />
            </FormField>
            <FormField label="تاريخ انتهاء الترخيص">
              <DateInput value={form.gov_rental_end} onChange={(e) => setForm((prev) => ({ ...prev, gov_rental_end: e.target.value }))} placeholder="اختر التاريخ" />
            </FormField>
          </div>
        ...
        <DialogContent className="max-w-5xl w-[95vw] p-0 overflow-hidden">
          ...
          <div className="relative bg-gradient-to-b from-card to-card/95 border-b border-border px-6 py-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-0.5">{selectedImage?.code}</p>
            </div>
            <Button variant="ghost" size="icon" onClick={() => setSelectedImage(null)} className="text-muted-foreground hover:text-foreground hover:bg-muted/80 rounded-lg">
              <X className="w-5 h-5" />
            </Button>
          </div>
          <div className="relative bg-black/5">
            <img src={selectedImage?.url} alt={selectedImage?.code} className="w-full h-auto max-h-[80vh] object-contain" />
          </div>
        </DialogContent>
      </Dialog>
    </div>
